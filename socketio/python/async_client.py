...
logger = logging.getLogger('Socketio.AsyncClient ')
...
async def main():
    ...
    try:
        await sio.connect('{}:{}'.format(options.host, options.port), transports='polling')
        ...
        while True:
            ...
            if not await sio.call('message', bytes(data)):
